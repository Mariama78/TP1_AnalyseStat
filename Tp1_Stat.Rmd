...
numeric_vars <- sapply(movies_released, is.numeric)
data_numeric <- movies_released[, numeric_vars]
lapply(names(data_numeric), function(x) {
  ggplot(movies_released, aes_string(y = x)) +
...
```{r}
# definissons une fonction qui nous permet d'imputer les valeurs aberrantes par la mediane

calculate_bounds <- function(data, k = 3) {
...
impute_outliers <- function(data, column_name, k = 1.5) {
  bounds <- calculate_bounds(data[[column_name]], k)
  data[[column_name]] <- ifelse(data[[column_name]] < bounds[1] | data[[column_name]] > bounds[2],
...
movies_released <- impute_outliers(movies_released,"runtime")
nrow(movies_released)
```
...
movies_released <- impute_outliers(movies_released,"popularity")
nrow(movies_released)
```
...
movies_released <- impute_outliers(movies_released,"vote_count")
nrow(movies_released)
```
...
revenue_counts <- data.frame(
  ...
  Nombre_de_films = sapply(revenue_thresholds * 1e6, function(threshold) sum(data$revenue > threshold, na.rm = TRUE))
)
